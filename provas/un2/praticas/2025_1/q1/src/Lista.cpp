...
bool Lista::inserirInicio(std::string x) {
  auto n = new No<std::string>(x);
  n->anterior = this->cabeca;
  n->proximo = this->cabeca->proximo;
...
std::string Lista::imprimir() const {
  ...
  for (; x != this->cauda; x = x->proximo) r << "<->" << x->valor;
  return r.str();
}

StatusDaLista Lista::checarConsistencia() const {
  ...
  if (this->quantidade == 0) {
    if (this->cabeca->proximo != this->cauda) return CABECA_CAUDA;
    if (this->cabeca != this->cauda->anterior) return CABECA_CAUDA;
  } else {
    int x = 0;
    for (auto i = this->cabeca->proximo; i != this->cauda; i = i->proximo) {
      if (i->proximo == nullptr || i->anterior == nullptr)
        return ENCADEAMENTO_INCORRETO;
      if (i->proximo->anterior != i) return ENCADEAMENTO_INCORRETO;
      if (i->anterior->proximo != i) return ENCADEAMENTO_INCORRETO;
      x++;
    }
    if (x != this->quantidade) return QUANTIDADE_DIFERENTE;
  }
  return OK;
}