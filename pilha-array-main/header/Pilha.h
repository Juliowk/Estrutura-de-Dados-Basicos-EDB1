...
class Pilha {
 ...
 public:
  ...
  T topo() const { return this->elementos[this->quantidade - 1]; }

  void empilhar(T elemento) {
    if (this->cheia()) throw std::overflow_error("Pilha cheia.");
  ...
  void desempilhar() {
    if (this->vazia()) throw std::underflow_error("Pilha vazia.");
